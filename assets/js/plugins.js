...
(function ($) {
  $.fn.bootstrapSwitch = function (options) {
    ...
    return this.each(function (e) {
      ...
      checkbox.prop(
        "checked",
        onValue.toString().toLowerCase() ==
          checkbox.val().toString().toLowerCase()
      );
      var disabled = checkbox.is(":disabled") ? " disabled" : "";

      var div = $(
        '<div class="btn-group btn-toggle" style="white-space: nowrap;"></div>'
      ).insertAfter(this);
      var on = $(
        '<button class="btn btn-success ' +
          size +
          disabled +
          '" style="float: none;display: inline-block;"></button>'
      )
        .html(onValue)
      ...
      function applyChange(isChecked) {
        if (isChecked) {
          ...
          on.attr("class", "btn active btn-" + onClass + size + disabled)
            .html(onLabel)
            .blur();
          off
            .attr("class", "btn btn-light " + size + disabled)
            .html(offLabel)
            .blur();
        } else {
          checkbox.val(offValue);
          on.attr("class", "btn btn-light " + size + disabled)
            .html(onLabel)
            .blur();
          off
            .attr("class", "btn active btn-" + offClass + size + disabled)
            .text(offLabel)
            .blur();
        }
  ...
  Plugin.prototype = {
    init: function () {
      ...
      var progressbar = jQuery(
        '<div class="progress mt-1 text-center" style="height:15px"><div class="progress-bar" style="width: 0%;" role="progressbar" aria-valuemin="0" aria-valuemax="100"></div></div>'
      );
    ...
    updateMeter: function (percentage) {
      ...
      if (percentage >= 100) {
        status = "bg-success";
        text =
          this.$elem.data("text-very-strong") || this.options.textVeryStrong;
